...
# endif
  ...
  if (!addr) {
    addr = __kleemill_find_unmapped_address(memory, kMmapMinAddr, max_addr, size);
    if (!addr) {
  ...
  memory = __kleemill_allocate_memory(memory, addr, size, fd_name, fd_offset);
  ...
  if (can_exec || !can_read || !can_write) {
    memory = __kleemill_protect_memory(memory, addr, size, can_read,
                                    can_write, can_exec);
  }
...
static Memory *SysMunmap(Memory *memory, State *state,
  ...
  memory = __kleemill_free_memory(memory, addr, size);
