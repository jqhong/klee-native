...
ref<klee::ConstantExpr> Executor::evalConstant(const Constant *c,
  ...
  if (const auto ce = dyn_cast<llvm::ConstantExpr>(c)) {
    ...
    if (const ConstantInt *ci = dyn_cast<ConstantInt>(c)) {
      ...
      ref<Expr> res = ConcatExpr::createN(static_cast<unsigned>(kids.size()),
                                          kids.data());
      ...
      for (unsigned i = cs->getNumOperands(); i != 0; --i) {
        ...
        uint64_t thisOffset = sl->getElementOffsetInBits(op), nextOffset =
            (op == cs->getNumOperands() - 1) ?
                sl->getSizeInBits() : sl->getElementOffsetInBits(op + 1);
        if (nextOffset - thisOffset > kid->getWidth()) {
          uint64_t paddingWidth = nextOffset - thisOffset - kid->getWidth();
          kids.push_back(
              ConstantExpr::create(0, static_cast<Expr::Width>(paddingWidth)));
        }
      ...
      for (auto i = static_cast<unsigned>(numOperands); i != 0; --i) {
        kids.push_back(evalConstant(cv->getOperand(i - 1), ki));
      }
      assert(Context::get().isLittleEndian() && "FIXME:Broken for big endian");
      ref<Expr> res = ConcatExpr::createN(static_cast<unsigned>(numOperands),
                                          kids.data());
      assert(
          isa<ConstantExpr>(res)
      ...
      const auto res = Expr::createPointer(
          reinterpret_cast<std::uint64_t>(arg_bb));
      return cast<ConstantExpr>(res);
    } else {
...
ref<ConstantExpr> Executor::evalConstantExpr(const llvm::ConstantExpr *ce,
  ...
  int numOperands = static_cast<int>(ce->getNumOperands());
  ...
  switch (ce->getOpcode()) {
    ...
    case Instruction::GetElementPtr: {
      ...
      for (gep_type_iterator ii = gep_type_begin(ce), ie = gep_type_end(ce);
          ii != ie; ++ii) {
        ref<ConstantExpr> indexOp = evalConstant(
            cast<Constant>(ii.getOperand()), ki);
        if (indexOp->isZero())
          continue;
...
#endif
          unsigned ElementIdx = static_cast<unsigned>(indexOp->getZExtValue());
        ...
        base = base->Add(
            indexOp->SExt(Context::get().getPointerWidth())->Mul(
                ConstantExpr::alloc(
                    APInt(
