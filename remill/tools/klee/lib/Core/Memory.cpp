...
ObjectHolder::~ObjectHolder(void) {
  if (os && --os->refCount == 0)
    delete os;
}
...
ObjectState::ObjectState(const ObjectState &os)
    : copyOnWriteOwner(0),
      ...
      concreteMask(
          os.concreteMask ? new BitArray(*os.concreteMask, os.size) : 0),
...
ArrayCache *ObjectState::getArrayCache(void) const {
...
const UpdateList &ObjectState::getUpdates(void) const {
  ...
  if (!updates.root) {
    ...
    size_t NumWrites = updates.head ? updates.head->getSize() : 0;
    std::vector<std::pair<ref<Expr>, ref<Expr> > > Writes(NumWrites);
    const UpdateNode *un = updates.head;
    for (size_t i = NumWrites; i != 0; un = un->next) {
      --i;
    ...
    size_t Begin = 0, End = Writes.size();
    for (; Begin != End; ++Begin) {
    ...
    if (isByteKnownSymbolic(i)) {
      ...
      if (!success) {
        klee_warning("Solver timed out when getting a value for external call, "
                     "byte %p+%" PRIu64 " will have random value",
...
void ObjectState::makeConcrete(void) {
  delete concreteMask;
  delete flushMask;
  delete[] knownSymbolics;
  concreteMask = 0;
...
void ObjectState::initializeToZero(void) {
  makeConcrete();
  memset(concreteStore, 0, size);
}

void ObjectState::initializeToRandom(void) {
  makeConcrete();
  for (size_t i = 0; i < size; i++) {
...
void ObjectState::fastRangeCheckOffset(ref<Expr> offset, size_t *base_r,
                                       size_t *size_r) const {
  *base_r = 0;
  *size_r = size;
}
...
bool ObjectState::isByteFlushed(size_t offset) const {
...
ref<Expr> ObjectState::read8(ref<Expr> offset) const {
  assert(
      !isa<ConstantExpr>(offset) && "constant offset passed to symbolic read8");
  size_t base, size;
  ...
  if (size > 4096) {
    ...
    klee_warning_once(
        0,
        "flushing %" PRIu64 " bytes on read, may be slow and/or crash: %s",
...
void ObjectState::write8(ref<Expr> offset, ref<Expr> value) {
  ...
  updates.extend(ZExtExpr::create(offset, Expr::Int64), value);
}

/***/

ref<Expr> ObjectState::read(ref<Expr> offset, Expr::Width width) const {
  offset = ZExtExpr::create(offset, Expr::Int64);

  // Check for reads at constant offsets.
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(offset)) {
    return read(CE->getZExtValue(), width);
  }
  ...
  for (unsigned i = 0; i != NumBytes; ++i) {
    uint64_t idx = Context::get().isLittleEndian() ? i : (NumBytes - i - 1);
    ref<Expr> Byte = read8(
        AddExpr::create(offset, ConstantExpr::create(idx, Expr::Int64)));
...
void ObjectState::print(void) const {
  ...
  llvm::errs() << "\tBytes:\n";
  for (unsigned i = 0; i < size; i++) {
    llvm::errs() << "\t\t[" << i << "]" << " concrete? " << isByteConcrete(i)
                 << " known-sym? " << isByteKnownSymbolic(i) << " flushed? "
                 << isByteFlushed(i) << " = ";
    ref<Expr> e = read8(i);
    llvm::errs() << e << "\n";
  }
