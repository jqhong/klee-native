...
cl::opt<ExternalCallPolicy> ExternalCalls("external-calls",
    cl::desc("Specify the external call policy"),
    cl::values(
        clEnumValN(ExternalCallPolicy::None, "none",
        ...
        clEnumValN(ExternalCallPolicy::All, "all",
            "All external function calls are allowed.  This concretizes any symbolic arguments in calls to external functions.") KLEE_LLVM_CL_VAL_END),
...
cl::opt<bool> AllExternalWarnings("all-external-warnings", cl::init(false),
    cl::desc("Issue a warning everytime an external call is made,"
        "as opposed to once per function (default=off)"), cl::cat(ExtCallsCat));
...
cl::opt<bool> ZeroSeedExtension("zero-seed-extension", cl::init(false),
    cl::desc(
        "Use zero-filled objects if matching seed not found (default=off)"),
    cl::cat(SeedingCat));
...
cl::opt<std::string> SeedTime("seed-time",
    cl::desc(
        "Amount of time to dedicate to seeds, before normal search (default=0s (off))"),
    cl::cat(SeedingCat));
...
cl::list<Executor::TerminateReason> ExitOnErrorType("exit-on-error-type",
    ...
    cl::values(clEnumValN(Executor::Abort, "Abort", "The program crashed"),
        ...
        clEnumValN(Executor::Unhandled, "Unhandled",
            "Unhandled instruction hit") KLEE_LLVM_CL_VAL_END), cl::ZeroOrMore,
    cl::cat(TerminationCat));

cl::opt<unsigned long long> MaxInstructions("max-instructions",
...
cl::opt<unsigned> MaxForks("max-forks",
    cl::desc("Only fork this many times (default=-1 (off))"), cl::init(~0u),
    cl::cat(TerminationCat));
...
cl::opt<unsigned> RuntimeMaxStackFrames("max-stack-frames",
    cl::desc(
        "Terminate a state after this many stack frames (default=8192).  Disable check with 0."),
    cl::init(8192), cl::cat(TerminationCat));

cl::opt<std::string> MaxInstructionTime("max-instruction-time",
    cl::desc(
...
cl::opt<double> MaxStaticForkPct("max-static-fork-pct", cl::init(1.),
    cl::desc(
        "Maximum percentage spent by an instruction forking out of the forking of all instructions (default=1.0 (always))"),
    cl::cat(TerminationCat));

cl::opt<double> MaxStaticSolvePct("max-static-solve-pct", cl::init(1.),
    cl::desc(
        "Maximum percentage of solving time that can be spent by a single instruction over total solving time for all instructions (default=1.0 (always))"),
    cl::cat(TerminationCat));

cl::opt<double> MaxStaticCPForkPct("max-static-cpfork-pct", cl::init(1.),
    cl::desc(
        "Maximum percentage spent by an instruction of a call path forking out of the forking of all instructions in the call path (default=1.0 (always))"),
    cl::cat(TerminationCat));

cl::opt<double> MaxStaticCPSolvePct("max-static-cpsolve-pct", cl::init(1.),
    cl::desc(
        "Maximum percentage of solving time that can be spent by a single instruction of a call path over total solving time for all instructions (default=1.0 (always))"),
    cl::cat(TerminationCat));
    ...
    llvm::cl::values(
        ...
        clEnumValN(FILE_COMPACT, "compact:file",
            "Log all instructions to file instructions.txt in format "
                "[inst_id]") KLEE_LLVM_CL_VAL_END), llvm::cl::CommaSeparated,
    cl::cat(DebugCat));
...
cl::opt<bool> DebugCheckForImpliedValues("debug-check-for-implied-values",
    cl::init(false), cl::desc("Debug the implied value optimization"),
    cl::cat(DebugCat));
...
const char *Executor::TerminateReasonNames[] = { [ Abort ] = "abort", [ Assert
    ] = "assert", [ BadVectorAccess ] = "bad_vector_access", [ Exec ] = "exec",
    [ External ] = "external", [ Free ] = "free", [ Model ] = "model",
    [ Overflow ] = "overflow", [ Ptr ] = "ptr", [ ReadOnly ] = "readonly",
    [ ReportError ] = "reporterror", [ User ] = "user", [ Unhandled ] = "xxx", };

Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    InterpreterHandler *ih) :
    Interpreter(opts), interpreterHandler(ih), searcher(0), externalDispatcher(
        new ExternalDispatcher(ctx)), solver(nullptr), memory(nullptr), statsTracker(
        0), pathWriter(0), symPathWriter(0), specialFunctionHandler(0), processTree(
        ...
        nullptr), code_cache(new ::native::BitCodeCache()) {
            ...
            coreSolverTimeout =
                maxCoreSolverTime && maxInstructionTime ?
...
Executor::setModule(std::vector<std::unique_ptr<llvm::Module>> &modules,
    ...
    semantics_module.reset(remill::LoadTargetSemantics(context).get());
    /* /Jiaqi */
    ...
    inst_lifter.reset(
            new remill::InstructionLifter(remill::GetTargetArch(context), *intrinsics));
    /* /Jiaqi */
...
void Executor::UpdateKModuleAfterLift(
    ...
    // std::unique_ptr<remill::Arch> tmp_arch;
    // tmp_arch.reset(remill::GetTargetArch(context));
    // remill::OptimizeModule(tmp_arch, semantics_module.get(), new_lifted_traces, guide);
    remill::OptimizeModule(remill::GetTargetArch(context), semantics_module.get(), new_lifted_traces, guide);
    /* /Jiaqi */
...
void Executor::Run(void) {
    while (auto task = NextTask()) {
        LOG(INFO) << "Interpreting Task!";
        runFunctionAsMain(task->first_func, task->argv, task->envp);
    }
}
...
void Executor::AddInitialTask(const std::string &state, const uint64_t pc,
  ...
  LOG(INFO) << "Initialized snapshotted task state";
}
...
void Executor::initializeGlobals(ExecutionState &state) {
  ...
  MemoryObject *errnoObj = addExternalObject(state, (void *) errno_addr,
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::IndirectBr: {
    ...
    if (const auto CE = dyn_cast<ConstantExpr>(address.get())) {
      const auto bb_address = (BasicBlock *) CE->getZExtValue(
    ...
    for (unsigned k = 0; k < numDestinations; ++k) {
      ...
      // check feasibility
      bool result;
      bool success __attribute__ ((unused)) = solver->mayBeTrue(state, e,
          result);
      assert(success && "FIXME: Unhandled solver failure");
      if (result) {
        targets.push_back(d);
    ...
    bool result;
    bool success __attribute__ ((unused)) = solver->mayBeTrue(state, errorCase,
        result);
    assert(success && "FIXME: Unhandled solver failure");
    if (result) {
  ...
  case Instruction::Switch: {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(cond)) {
      ...
      for (std::map<ref<Expr>, BasicBlock *>::iterator it =
          expressionOrder.begin(), itE = expressionOrder.end(); it != itE;
          ++it) {
  ...
  case Instruction::Call: {
    ...
    if (f) {
      ...
      if (fType != fpType) {
        ...
        unsigned i = 0;
        for (std::vector<ref<Expr> >::iterator ai = arguments.begin(), ie =
  ...
  case Instruction::GetElementPtr: {
    ...
    for (std::vector<std::pair<unsigned, uint64_t> >::iterator it =
        kgepi->indices.begin(), ie = kgepi->indices.end(); it != ie; ++it) {
...
void Executor::computeOffsets(KGEPInstruction *kgepi, TypeIt ib, TypeIt ie) {
  ...
  for (TypeIt ii = ib; ii != ie; ++ii) {
    if (StructType *st = dyn_cast<StructType>(*ii)) {
      ...
      auto elementSize = kmodule->targetData->getTypeStoreSize(
          ptr->getElementType());
...
static native::AreaAllocator gConstantsAllocator(native::kAreaRW,
    native::kAreaConstantTable);
...
void Executor::bindModuleConstants(llvm::Module *mod) {
  ...
  if (!kmodule->constantTable) {
    kmodule->constantTable =
        reinterpret_cast<Cell *>(gConstantsAllocator.Allocate(needed_bytes));
  } else {
...
//  initTimers();
  ...
  while (!continuations.empty()) {
    ...
    LOG(INFO) << "Finished state, continuation stack size is "
        << continuations.size();
  }
...
void Executor::callExternalFunction(ExecutionState &state, KInstruction *target,
  ...
  if (!errnoValue) {
    terminateStateOnExecError(state,
        "external call with errno value symbolic: " + function->getName());
    return;
  }
...
void Executor::executeAlloc(ExecutionState &state, ref<Expr> size, bool isLocal,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    MemoryObject *mo = memory->allocate(CE->getZExtValue(), isLocal, /*isGlobal=*/
    ...
    if (fixedSize.second) {
      ...
      (void) success;
      bool res;
      success = solver->mustBeTrue(*fixedSize.second, EqExpr::create(tmp, size),
          res);
      assert(success && "FIXME: Unhandled solver failure");
      (void) success;
      if (res) {
...
void Executor::executeMemoryOperation(ExecutionState &state, bool isWrite,
  ...
  Expr::Width type =
      (isWrite ?
          value->getWidth() : getWidthForLLVMType(target->inst->getType()));
...
void Executor::runFunctionAsMain(Function *f,
  ...
  LOG(INFO) << "Preparing to run main function";
  ...
  LOG(INFO) << "Initialized globals in state";
...
size_t Executor::getAllocationAlignment(const llvm::Value *allocSite) const {
  ...
  if (const GlobalValue *GV = dyn_cast<GlobalValue>(allocSite)) {
    ...
    if (const GlobalVariable *globalVar = dyn_cast<GlobalVariable>(GV)) {
      ...
      llvm::PointerType *ptrType = dyn_cast<llvm::PointerType>(
          globalVar->getType());
    ...
    klee_warning_once(fn != NULL ? fn : allocSite,
        "Alignment of memory from call \"%s\" is not "
            "modelled. Using alignment of %zu.", allocationSiteName.c_str(),
        forcedAlignment);
  ...
  if (alignment == 0) {
    ...
    if (type->isSized()) {
      ...
      klee_warning_once(allocSite, "Cannot determine memory alignment for "
          "\"%s\". Using alignment of %zu.", allocationSiteName.c_str(),
          forcedAlignment);
      alignment = forcedAlignment;
    }
  }
