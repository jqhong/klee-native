...
void SeedInfo::patchSeed(const ExecutionState &state, ref<Expr> condition,
  ...
  for (std::vector<ref<ReadExpr> >::iterator it = reads.begin(), ie =
      reads.end(); it != ie; ++it) {
    ReadExpr *re = it->get();
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(re->index)) {
      directReads.insert(
          std::make_pair(re->updates.root,
                         static_cast<uint32_t>(CE->getZExtValue(32))));
    }
  }

  for (const auto &read_info : directReads) {
    const Array *array = read_info.first;
    unsigned i = read_info.second;
    ...
    if (it2 != assignment.bindings.end()) {
      ...
      if (res) {
        ...
        tmp.addConstraint(
            EqExpr::create(read,
                           ConstantExpr::alloc(it2->second[i], Expr::Int8)));
      } else {
  ...
  for (Assignment::bindings_ty::iterator it = assignment.bindings.begin(), ie =
      assignment.bindings.end(); it != ie; ++it) {
  ...
  {
    bool res;
    bool success = solver->mayBeTrue(state, assignment.evaluate(condition),
                                     res);
    assert(success && "FIXME: Unhandled solver failure");
    (void) success;
